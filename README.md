...
# ハマった点、苦労した点  
### ハマった点  
- イベントの登録・削除をデータベースに反映させる機能の実装  
- データベースに登録されたイベントをカレンダー上に表示する機能の実装  
- JavaScriptコードの肥大化に対する解決策として、import/exportを活用したモジュール分割の導入  
  (機能別にファイルを整理したことで、コードの可読性が向上し、非常に爽快でした！)
- 目的の機能を実装するために試行錯誤する過程  
  (繰り返し頭の中でアイデアを練り、実装への突破口が見つかったときの充足感！)  
### 苦労した点  
- カレンダー上の日付クリックによる自動入力機能  
- MVCモデルにおけるView部分とController部分の適切な連携  
  (フロントエンドとバックエンドの紐づけがとても苦労しました)
